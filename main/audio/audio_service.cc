...
void AudioService::PushTaskToEncodeQueue(AudioTaskType type, std::vector<int16_t>&& pcm) {
    ...
    // 带超时的等待，避免编码队列满时阻塞死等导致任务看门狗超时重启
    bool queue_available = audio_queue_cv_.wait_for(lock, std::chrono::milliseconds(100),
...
void AudioService::CheckAndUpdateAudioPowerState() {
    ...
    if (output_elapsed > AUDIO_POWER_TIMEOUT_MS && codec_->output_enabled()) {
        // Before disabling output, check if there is still pending audio in the pipeline.
        // This prevents choppy/cut-off "goodbye" TTS when the server sends exit messages
        // slightly after the local idle timer fires.
        bool has_pending_audio = false;
        {
            std::lock_guard<std::mutex> lock(audio_queue_mutex_);
            has_pending_audio = !audio_decode_queue_.empty() || !audio_playback_queue_.empty();
        }
        if (has_pending_audio) {
            // Reset the timer so new output packets can arrive and play uninterrupted
            last_output_time_ = std::chrono::steady_clock::now();
        } else {
